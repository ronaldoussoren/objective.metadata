...
parser.add_argument("--ini-section", metavar="SECTION", action="append", default=[],
                    help="section to use (default: use all sections)")

subparsers = parser.add_subparsers(dest="command")
scan_command = subparsers.add_parser("scan", help="Scan header files")
scan_command.add_argument("--sdk-root", help="Use the given SDK", metavar="DIR", default="/")
scan_command.add_argument("--libclang", help="Use libclang at path", metavar="FILE", default=None)
scan_command.add_argument("--min-deployment-ver", help="Use the given SDK", metavar="DIR", default=None)
scan_command.add_argument("--arch", help="Use the given processor architecture", metavar="ARCH", default="x86_64")
...
def parse_ini(ini_file, ini_sections):
    ...
    for section in ini_sections:
        ...
        if cfg.has_option(section, "compiled"):
            ...
            info["compiled"] = os.path.join(ini_dir, "..", "Lib", info["python-package"].replace('.', os.path.sep),
                                            "_metadata.py")
        ...
        if args.command == "scan":
            ...
            for arch in args.arch.split(','):
                ...
                for sdk in args.sdk_root.split(','):
                    ...
                    min_deploy = None if args.min_deployment_ver is None else float(str(args.min_deployment_ver))
                    if min_deploy is None:
                        current_os_ver = float('.'.join(platform.mac_ver()[0].split(".")[0:2]))
